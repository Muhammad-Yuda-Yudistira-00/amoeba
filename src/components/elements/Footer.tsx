...
const Footer: React.FC<FooterProps> = ({code}) => {
	...
	return(
		<footer className="py-2 px-2">
			<div>
				<p className={`text-xs md:text-lg text-amber-200 font-geistMono`}>Your checklist active still in <span className="text-xs md:text-base">{checklistData ? timeToHuman(checklistData?.data.expiredAt) : "-"} / 31</span> days.</p>
			</div>
			<div>
