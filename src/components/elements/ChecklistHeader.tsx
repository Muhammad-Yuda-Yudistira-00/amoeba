...
export default function ChecklistHeader({checklist, code, setChecklist}: {checklist: Checklist | null, code: string, setChecklist: React.Dispatch<React.SetStateAction<Checklist | null>>}) {
	return(
		<div className="pb-4 md:pb-8 px-4">
			...
			<div className="bg-yellow-400 text-center rounded-sm md:rounded-es-full md:rounded-se-full px-2 md:px-8">
				<small contentEditable="true" dangerouslySetInnerHTML={{ __html: checklist?.description ? checklist.description : 'Descript..' }} onBlur={(e) => handleChangeDescription(e, code, setChecklist)} className="text-sm md:text-base px-4 py-2 text-stone-600" />
			</div>
		</div>
	)
}